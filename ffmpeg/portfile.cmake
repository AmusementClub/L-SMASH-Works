...
vcpkg_from_github(
    ...
    PATCHES
        ...
        0021-MPEG-2-Video.patch
        0022-Properly-check-chroma-subsampling-in-CUVID-decoder.patch
        0023-Revert-avcodec-h264_parser-set-missing-pts-for-top-b.patch
        0024-Revert-vc-1-Optimise-parser-with-special-attention-t.patch
        ${PATCHES}
)
...
if (VCPKG_BUILD_TYPE STREQUAL "debug")
    ...
    set(OPTIONS "${OPTIONS} --disable-debug")
endif()
...
if("network" IN_LIST FEATURES)
    set(OPTIONS "${OPTIONS} --enable-network")
else()
    set(OPTIONS "${OPTIONS} --disable-network")
endif()

if("muxers" IN_LIST FEATURES)
    set(OPTIONS "${OPTIONS} --enable-muxers")
else()
    set(OPTIONS "${OPTIONS} --disable-muxers")
endif()

if("demuxers" IN_LIST FEATURES)
    set(OPTIONS "${OPTIONS} --enable-demuxers")
else()
    set(OPTIONS "${OPTIONS} --disable-demuxers")
endif()

if("encoders" IN_LIST FEATURES)
    set(OPTIONS "${OPTIONS} --enable-encoders")
else()
    set(OPTIONS "${OPTIONS} --disable-encoders")
endif()

if("decoders" IN_LIST FEATURES)
    set(OPTIONS "${OPTIONS} --enable-decoders")
else()
    set(OPTIONS "${OPTIONS} --disable-decoders")
endif()
